import { Button, Card, CardContent, CardMedia, Checkbox, FormControlLabel, List, ListItem, ListItemIcon, ListItemText, TextField, Typography } from '@mui/material'
...
function Account() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      const fetchData = async () => {
        const response = await fetch(`http://localhost:8082/get-order-details/${userId}`);
  ...
  const iconMap = {
    'Personal Information': <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24" fill="none">
    ...
    'Order': <svg xmlns="http://www.w3.org/2000/svg" fill="#000000" width="22px" height="22px" viewBox="12 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve">
      <g>
    ...
    'Logout': <svg xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" viewBox="0 0 24 24" fill="none">
        ...
        <div className='py-32 px-10 bg-white shadow-lg rounded'>
          ...
          <List>
            {options.map((text, index) => (
              <ListItem button key={text} onClick={() => handleOptionSelect(text)}>
                <ListItemIcon>
                  {iconMap[text]}
                </ListItemIcon>
                <ListItemText
                  primary={text}
                  className={`font-bold block py-2 pr-4 pl-3 duration-200 ${selectedOption === text ? "text-orange-700" : "text-gray-700"
            ...
            <div>
              <form >
                ...
                <div className='hidden flex justify-center mt-8'>
                  <Button
                    style={{ borderRadius: '13px', backgroundColor: '#7827ed', width: '200px', height: '30px' }}
            ...
            user && selectedOption == 'Order' && (
              <div>
                {orderDetails.map((orderDetail) => (
                  <ListItem>
                    <Card key={orderDetail.id} style={{ marginBottom: '10px', display: 'flex' }}>
