...
function ProductList() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await fetch(`http://localhost:8082/get-all-products/${userId}/${cuisineType}`);
      ...
      <div className="mt-6">
        <div className="grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-12 xl:gap-y-14">
          {products.map((product) => (
            <div key={product.id} className="shadow-lg rounded-lg group relative flex flex-col">
              ...
              <div className="flex-grow flex flex-col justify-between items-center p-4">
                ...
                <div className="mt-2">
                  <Typography style={{ color: '#616161', marginBottom: '2px', fontSize: '12px', fontFamily: 'unset', textAlign: 'center' }}>
                    {product.description}
                  </Typography>
                </div>
